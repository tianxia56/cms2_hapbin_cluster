...
generate_map_and_haps() {
    ...
    for pop_id in "${pop_ids[@]}"; do
        python3 hapbin.map.py "$sim_id" "$pop_id" "$pop1" "$config_file" "$max_pop_id"
    done
    end_time=$(date +%s)
    runtime=$((end_time - start_time))
    echo "Runtime for simulation ID $sim_id, make map and haps: $runtime seconds" >> "hapbin/hapbin.runtime.txt"
}
...
run_xpehhbin() {
    ...
    for pop2 in $(seq 1 $max_pop_id); do
        if [[ $pop2 -ne $pop1 ]]; then
            ...
            end_time=$(date +%s)
            runtime=$((end_time - start_time))
            echo "Runtime for simulation ID $sim_id, 1vs$pop2: $runtime seconds" >> "hapbin/hapbin.runtime.txt"
        fi
    done
}
