...
run_selscan_xpehh() {
    ...
    start_time=$(date +%s)
    selscan --xpehh --tped $tped_file1 --tped-ref $tped_file2 --out two_pop_stats/sel.${sim_id}_${pop1}_vs_${pop2} --threads 8
    end_time=$(date +%s)
    runtime=$((end_time - start_time))
    echo "xpehh sel runtime for ${sim_id}_${pop1}_vs_${pop2}: $runtime seconds" >> two_pop_stats/xpehh.runtime.txt
}

# Monitor the sel directory for new TPED files and run selscan xpehh commands on them
processed_files=()
while true; do
    for ((sim_id=0; sim_id<=selected_simulation_number; sim_id++)); do
        tped_file1="sel/sel.hap.${sim_id}_0_${pop1}.tped"
        tped_file2="sel/sel.hap.${sim_id}_0_${pop2}.tped"
...
echo "XPEHH statistics for selected simulations have been calculated and saved in the two_pop_stats directory."
