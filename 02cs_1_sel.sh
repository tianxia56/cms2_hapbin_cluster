...
run_command() {
    ...
    while true; do
        start_time=\$(date +%s)
        env COSI_SAVE_SAMPLED=../runtime/cosi.sel.1.csv COSI_NEWSIM=1 COSI_MAXATTEMPTS=1000000 coalescent -p ../\${demographic_model}-\${sim_id}.par -v -g --genmapRandomRegions --drop-singletons .25 --tped ../sel/sel.\${output_name} -n 1 -M -r 0 &
        local cmd_pid=\$!
    ...
    echo "\$sim_id" >> ../runtime/cosi.sel.1.csv

    # Remove the copied .par file
    rm ../\${demographic_model}-\${sim_id}.par
}

# Loop to run the command for each output name in increments of 5
for a in \$(seq 0 5 \$selected_simulation_number); do
    output_name="hap.\$a"
