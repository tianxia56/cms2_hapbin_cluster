...
run_command() {
    ...
    while true; do
        start_time=\$(date +%s)
        env COSI_SAVE_SAMPLED=../runtime/cosi.sel.5.csv COSI_NEWSIM=1 COSI_MAXATTEMPTS=1000000 coalescent -p ../\${demographic_model}-\${sim_id}.par -v -g --genmapRandomRegions --drop-singletons .25 --tped ../sel/sel.\${output_name} -n 1 -M -r 0 &
        local cmd_pid=\$!
    ...
    echo "\$sim_id" >> ../runtime/cosi.sel.5.csv

    # Remove the copied .par file
    rm ../\${demographic_model}-\${sim_id}.par
}

# Loop to run the command for each output name in increments of 5 starting from 4
for a in \$(seq 4 5 \$selected_simulation_number); do
    output_name="hap.\$a"
    run_command \$output_name \$a "\$demographic_model" "\$selective_sweep"
done

# Restart the loop from the last sequence if needed
for a in \$(seq \$((selected_simulation_number + 5)) 5 \$selected_simulation_number); do
    output_name="hap.\$a"
    run_command \$output_name \$a "\$demographic_model" "\$selective_sweep"
done
EOF
