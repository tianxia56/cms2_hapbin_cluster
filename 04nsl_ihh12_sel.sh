...
# Function to get the maximum simulation ID from the CSV file
get_max_sim_id() {
    max_sim_id=$(awk -F, 'NR > 1 {if ($2 > max) max=$2} END {print max}' runtime/cosi.${path}.runtime.csv)
    echo $max_sim_id
}
...
monitor_new_sim_ids() {
    ...
    while true; do
        ...
        max_sim_id=$(get_max_sim_id)
        if [ $max_sim_id -eq $((selected_simulation_number + 1)) ]; then
            echo "Max sim_id reached. Monitoring for another 30 minutes..."
            sleep 1800
            echo "Stopping script after additional 30 minutes of monitoring."
            break
        fi
